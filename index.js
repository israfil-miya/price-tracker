...
const runPriceTracking = async (db, items) => {
  try {
    for (const item of items) {
      await checkAndUpdateItem(
        db,
        new ObjectId(Buffer.from(item._id.id).toString('hex')).toString(),
      )
    }
  } catch (error) {
...
const startPriceTracking = async () => {
  try {
    ...
    while (true) {
      ...
      console.log(
        `Starting price tracking for ${items.length} items using ${numThreads} threads...`,
      )
      ...
      for (let i = 0; i < numThreads; i++) {
        ...
        worker.on('exit', (code) => {
          if (code !== 0) {
            console.error(
              `Price tracking worker stopped with exit code ${code}`,
            )
          }
        })
      }
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, parseInt(process.env.INTERVAL_TIME_IN_MS)),
      )
    }
  } catch (error) {
